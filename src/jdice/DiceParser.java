...
public class DiceParser{
	...
	private void munchWhiteSpace() { // bo dau //
	    int index=0;
	    char curr;
	...
	public Integer getInt(){
	    return readInt(); // thieu return
	}
	public Integer readInt(){
	    int index=0;
	    char curr;
	    munchWhiteSpace(); // thieu dau ;
	    while(index<buff.length()){
		curr=buff.charAt(index);
	    ...
	    try{
		Integer ans;
		ans=Integer.parseInt(buff.substring(0, //IntegerparseInt thieu dau '.'
						    index));
		buff=buff.delete(0,index);
		return ans;
	    }
    ...
    **/
    ...
    private static Vector<DieRoll> parseRollInner(StringStream ss,
						   Vector<DieRoll> v){
	Vector<DieRoll> r=parseXDice(ss); //Vector<DieRoll thieu dau '>'
	if(r==null) {
	    return null;
	}
	v.addAll(r);
